...
- `B_PARALYSIS_SPEED` - Type: Generation Toggle
    - Paralysis reduces the afflicted Pokemon's Speed by: [[More Info]](https://bulbapedia.bulbagarden.net/wiki/Paralysis_(status_condition)#Effect)
        - \>= `GEN_7`: 50%
        - <= `GEN_6`: 75%
...
- `B_CONFUSION_SELF_DMG_CHANCE` - Type: Generation Toggle
    - The likelihood of a Confused Pokemon hurting themselves in confusion is: [[More Info]](https://bulbapedia.bulbagarden.net/wiki/Confusion_(status_condition)#Effect)
        - \>= `GEN_7`: 33.3%
        - <= `GEN_6`: 50%
...
- `B_GHOSTS_ESCAPE` - Type: Generation Toggle
    - Abilities like Shadow Tag or moves like Mean Look fail on Ghost-type Pokémon. They can also escape any Wild Battle. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_PARALYZE_ELECTRIC` - Type: Generation Toggle
    - Electric-type Pokémon can't be paralyzed. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_POWDER_GRASS` - Type: Generation Toggle
    - Grass-type Pokémon are immune to powder and spore moves. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_STEEL_RESISTANCES` - Type: Generation Toggle
    - Steel-type Pokémon are resistant to Dark-type and Ghost-type moves. [More Info] 
        - \>= `GEN_6`: False
        - <= `GEN_5`: True
...
- `B_PRANKSTER_DARK_TYPES` - Type: Generation Toggle
    - Prankster-elevated status moves do not affect Dark type Pokémon. [More Info] 
        - \>= `GEN_7`: True
        - <= `GEN_6`: False
...
- `B_SHEER_COLD_IMMUNITY` - Type: Generation Toggle
    - Ice-types are immune to Sheer Cold. [More Info] 
        - \>= `GEN_7`: True
        - <= `GEN_6`: False
...
- `B_ROOST_PURE_FLYING` - Type: Generation Toggle
    - When a pure Flying-type Pokemon uses Roost, their type changes to [More Info] 
        - \>= `GEN_7`: Normal
        - <= `GEN_6`: Type-less
...
- `B_BINDING_TURNS` - Type: Generation Toggle
    - Binding moves last for a duration of [More Info] 
        - \>= `GEN_5`: 4-5 turns
        - <= `GEN_4`: 2-5 turns
...
- `B_UPROAR_TURNS` - Type: Generation Toggle
    - Uproar lasts for a duration of [More Info] 
        - \>= `GEN_5`: 3 turns
        - <= `GEN_4`: 2-5 turns
...
- `B_DISABLE_TURNS` - Type: Generation Toggle
    - The effects of Disable last for [More Info] 
        - \>= `GEN_5`: 4 turns
        - = `GEN_4`: 4 - 7 turns
        - = `GEN_3`: 2 - 5 turns
...
- `B_TAILWIND_TURNS` - Type: Generation Toggle
    - The effects of Tailwind last for [More Info] 
        - \>= `GEN_5`: 4 turns
        - <= `GEN_4`: 3 turns
...
- `B_SLEEP_TURNS` - Type: Generation Toggle
    - The duration of the Sleep status lasts for [More Info] 
        - \>= `GEN_5`: 1 - 3 turns
        - <= `GEN_4`: 2 - 5 turns
...
- `B_TAUNT_TURNS` - Type: Generation Toggle
    - The effects of Taunt lasts for [More Info] 
...
- `B_SPORT_TURNS` - Type: Generation Toggle
    - The effects of Mud / Water Sport last for[More Info] 
...
- `B_MEGA_EVO_TURN_ORDER` - Type: Generation Toggle
    - A Pokemon's turn order is determined by its Speed [More Info] 
...
- `B_RECALC_TURN_AFTER_ACTIONS` - Type: Generation Toggle
    - Switching or using a move will affect the current turn's order of actions. [More Info] 
        - \>= `GEN_8`: True
        - <= `GEN_7`: False
...
- `B_FAINT_SWITCH_IN` - Type: Generation Toggle
    - When a Pokemon faints in battle, it is replaced [More Info] 
...
- `B_UPDATED_MOVE_DATA` - Type: Generation Toggle
    - All moves use the data (Power, Accuracy, PP, stat change, targets, chance of secondary effects) from the generation that is set here. [More Info] 
...
- `B_UPDATED_MOVE_TYPES` - Type: Generation Toggle
    - All moves use the types from the generation that is set here. [More Info] 
...
- `B_PHYSICAL_SPECIAL_SPLIT` - Type: Generation Toggle
    - Moves are assigned a classification (Physical / Special / Status) [More Info] 
...
- `B_RECOIL_IF_MISS_DMG` - Type: Generation Toggle
    - When the moves High Jump Kick and Jump Kick miss their target, the user takes damage equal to 1/2 of their total HP [More Info] 
        - \>= `GEN_5`: at all times
...
- `B_KLUTZ_FLING_INTERACTION` - Type: Generation Toggle
    - Pokemon with the Ability Klutz cannot use Fling. [More Info] 
        - \>= `GEN_5`: True
        - <= `GEN_4`: False
...
- `B_UPDATED_CONVERSION` - Type: Generation Toggle
    - Conversion changes the user's type to match the type of [More Info] 
...
- `B_PP_REDUCED_BY_SPITE` - Type: Generation Toggle
    - The effects of Spite reduces the target's last move's PP by [More Info] 
        - \>= `GEN_4`: 4
        - = `GEN_3`: 2 to 5
...
- `B_TOXIC_NEVER_MISS` - Type: Generation Toggle
    - If Toxic is used by a Poison-type Pokemon, it will never miss. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_MINIMIZE_DMG_ACC` - Type: Generation Toggle
    - Moves that causes double damage to minimized Pokémon will also skip accuracy checks[More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_BLIZZARD_HAIL` - Type: Generation Toggle
    - Blizzard bypasses accuracy checks if it's hailing. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_SHEER_COLD_ACC` - Type: Generation Toggle
    - If the user is an Ice-type, the base chance of Sheer Cold hitting the target is [More Info] 
        - \>= `GEN_7`: 20% 
        - <= `GEN_6`: 30%
...
- `B_FELL_STINGER_STAT_RAISE` - Type: Generation Toggle
    - When damage from Fell Stinger causes the target to faint, the user's Attack is raised by [More Info] 
        - \>= `GEN_7`: 3 stages
        - <= `GEN_6`: 2 stages
...
- `B_KINGS_SHIELD_LOWER_ATK` - Type: Generation Toggle
    - The effect's of King's Shield reduces the Attack of the attacking Pokemon by [More Info] 
        - \>= `GEN_8`: 1 stage
        - <= `GEN_7`: 2 stages
...
- `B_SPEED_BUFFING_RAPID_SPIN` - Type: Generation Toggle
    - Rapid Spin raises the user's Speed by 1 stage. [More Info] 
        - \>= `GEN_8`: True
        - <= `GEN_7`: False
...
- `B_CHARGE_SPDEF_RAISE` - Type: Generation Toggle
    - Charge raises the user's Special Defense by 1 stage. [More Info] 
        - \>= `GEN_5`: True
        - <= `GEN_4`: False
...
- `B_MINIMIZE_EVASION` - Type: Generation Toggle
    - Minimize raises evasion by [More Info] 
        - \>= `GEN_5`: 2 stages
        - <= `GEN_4`: 1 stage
...
- `B_GROWTH_STAT_RAISE` - Type: Generation Toggle
    - Growth's mechanics change between generations. [More Info] 
...
- `B_SOUND_SUBSTITUTE` - Type: Generation Toggle
    - Sound moves bypass Substitute. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_INCINERATE_GEMS` - Type: Generation Toggle
    - Incinerate can destroy Gems. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_CAN_SPITE_FAIL` - Type: Generation Toggle
    - Spite fails if the target's last move only has 1 remaining PP. [More Info] 
        - \>= `GEN_4`: False
        - = `GEN_3`: True
...
- `B_CRASH_IF_TARGET_IMMUNE` - Type: Generation Toggle
    - If the target of Jump Kick or High Jump Kick is immune, the user will be dealt damage as if the move missed. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_MEMENTO_FAIL` - Type: Generation Toggle
    - Memento fails if there is no target or if the target is protected or behind substitute. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_GLARE_GHOST` - Type: Generation Toggle
    - Ghost-type Pokemon are immune to Glare. [More Info] 
        - \>= `GEN_4`: False 
        - = `GEN_3`: True 
...
- `B_SKILL_SWAP` - Type: Generation Toggle
    -  After Skill Swap was successful, if an Ability that normall activates on switch in was affected, it immediately activates. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_BRICK_BREAK` - Type: Generation Toggle
    - Brick Break gains new properties between generations[More Info] 
...
- `B_WISH_HP_SOURCE` - Type: Generation Toggle
    - Wish heals half of the Max HP of the [More Info] 
        - \>= `GEN_5`: user
        - <= `GEN_4`: target
...
- `B_RAMPAGE_CANCELLING` - Type: Generation Toggle
    - When a Pokemon is rampaging and its attack fails, the rampage will [More Info] 
...
- `B_HEAL_BLOCKING` - Type: Generation Toggle
    - Heal Block's mechanics change between generations. [More Info] 
...
- `B_ROOTED_GROUNDING` - Type: Generation Toggle
    - Ingrain causes the affected Pokémon to become grounded. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_METRONOME_MOVES` - Type: Generation Toggle
    - Metronome will randomly choose from the generation that is defined here, and every generation prior to it. [More Info] 
...
- `B_TELEPORT_BEHAVIOR` - Type: Generation Toggle
    - Teleport allows the user to swap out with another party member. [More Info] 
        - \>= `GEN_7`: True
        - <= `GEN_6`: False
...
- `B_DARK_VOID_FAIL` - Type: Generation Toggle
    - When a Pokemon other than Darkrai uses the move Dark Void, the move will fail. [More Info] 
        - \>= `GEN_7`: True 
        - <= `GEN_6`: False
...
- `B_BURN_HIT_THAW` - Type: Generation Toggle
    - When a non-Fire type damaging move with a chance to cause Burn hits a Frozen target, there is a chance the target will be defrosted. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_DEFOG_CLEARS_TERRAIN` - Type: Generation Toggle
    - Defog clears active Terrain. [More Info] 
        - \>= `GEN_8`: True
        - <= `GEN_7`: False
...
- `B_STOCKPILE_RAISES_DEFS` - Type: Generation Toggle
    - Stockpile raises Defense and Sp.Defense stats, which are negated once Spit Up or Swallow is used. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_ABILITY_WEATHER` - Type: Generation Toggle
    - Sand Stream, Snow Warning, Drought, and Drizzle summon their respective weather for N turns. [More Info] 
        - \>= `GEN_6`: 5
        - <= `GEN_5`: ∞
...
- `B_GALE_WINGS` - Type: Generation Toggle
    - Gale Wings activates when the user's HP is at N. [More Info] 
        - \>= `GEN_7`: 100%
        - <= `GEN_6`: Any
...
- `B_STANCE_CHANGE_FAIL` - Type: Generation Toggle
    - Stance Change fails if the Pokémon is unable to use a move because of confusion, paralysis, etc.  [More Info] 
        - \>= `GEN_7`: True
        - <= `GEN_6`: False
...
- `B_SHADOW_TAG_ESCAPE` - Type: Generation Toggle
    - When both sides of a battle are affected by Shadow Tag, both sides are able to switch. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_MOODY_ACC_EVASION` - Type: Generation Toggle
    - Moody can modify evasion and/or accuracy. [More Info] 
        - \>= `GEN_8`: False
        - <= `GEN_7`: True
...
- `B_FLASH_FIRE_FROZEN` - Type: Generation Toggle
    - Flash Fire triggers when user is frozen. [More Info] 
        - \>= `GEN_5`: True
        - <= `GEN_4`: False
...
- `B_SYNCHRONIZE_NATURE` - Type: Generation Toggle
    - When the first Pokemon in the party has Synchronize, the odds that wild Pokemon will have a Nature that matches the first Pokemon are [More Info] 
        - \>= `GEN_8`: 100%
        - <= `GEN_7`: 50%
...
- `B_SYNCHRONIZE_TOXIC` - Type: Generation Toggle
    -  When a Pokemon with Synchronize is badly poisoned, Synchronize will inflict the opponent with [More Info] 
...
- `B_UPDATED_INTIMIDATE` - Type: Generation Toggle
    - Intimidate activates Rattled, and is blocked by Inner Focus, Scrappy, Own Tempo, and Oblivious. [More Info] 
        - \>= `GEN_8`: True
        - <= `GEN_7`: False
...
- `B_OBLIVIOUS_TAUNT` - Type: Generation Toggle
    - Oblivious blocks the effect of Taunt. [More Info] 
        - \>= `GEN_6`: True
        - <= `GEN_5`: False
...
- `B_STURDY` - Type: Generation Toggle
    - If a Pokemon has Sturdy and has full HP and is hit by a non one-hit knockout move that would knock it out [More Info] 
...
- `B_PLUS_MINUS_INTERACTION` - Type: Generation Toggle
    - Plus and Minus are activated by an ally having [More Info] 
        - \>= `GEN_5`: Plus or Minus
...
- `B_WEATHER_FORMS` - Type: Generation Toggle
    - When a Pokemon has Flower Gift or Forecast, they can only transform when they have those abilities. When those abilities are lost or disabled, those Pokemon revert into their base form. [More Info] 
        - \>= `GEN_5`: True
        - <= `GEN_4`: False
...
- `B_SYMBIOSIS_GEMS` - Type: Generation Toggle
    - After a Pokemon consumes a Gem, an item from an ally with Symbiosis is transferred [More Info] 
...
- `B_ABSORBING_ABILITY_STRING` - Type: Generation Toggle
    - The message that displays when abilities absorb moves of a certain type is [More Info] 
...
- `B_HP_BERRIES` - Type: Generation Toggle
    - Berries which restore HP activate [More Info] 
...
- `B_BERRIES_INSTANT` - Type: Generation Toggle
    - Berries that Pokemon are eligible for consumption will be consumed [More Info] 
...
- `B_CONFUSE_BERRIES_HEAL` - Type: Generation Toggle
    - Confusion berries mechanics change between generations. [More Info] 
...
- `B_MENTAL_HERB` - Type: Generation Toggle
    - Mental Herb cures Infatuation [More Info] 
...
- `B_TRAINERS_KNOCK_OFF_ITEMS` - Type: True / False
    - When `TRUE`, opposing trainers can steal the player's items. Items that are [berries](https://bulbapedia.bulbagarden.net/wiki/Berry) are restored after battle.  [More Info] 
...
- `B_SOUL_DEW_BOOST` - Type: Generation Toggle
    - Soul Dew boosts Latios and Latias' [More Info] 
...
- `B_NET_BALL_MODIFIER` - Type: Generation Toggle
    - Net Ball's catch multiplier is [More Info] 
        - \>= `GEN_7`: 5x
        - <= `GEN_6`: 3x
...
- `B_DIVE_BALL_MODIFIER` - Type: Generation Toggle
    - Dive Ball's catch multiplier is applied when the player is diving, [More Info] 
...
- `B_REPEAT_BALL_MODIFIER` - Type: Generation Toggle
    - Repeat Ball's catch multiplier is [More Info] 
        - \>= `GEN_7`: 3.5x
        - <= `GEN_6`: 3x
...
- `B_TIMER_BALL_MODIFIER` - Type: Generation Toggle
    - Every turn, Timer Ball's catch multiplier increases by [More Info] 
        - \>= `GEN_5`: 0.3
        - <= `GEN_4`: 0.1
...
- `B_DUSK_BALL_MODIFIER` - Type: Generation Toggle
    - Dusk Ball's catch multiplier is [More Info] 
        - \>= `GEN_7`: 3x
        - <= `GEN_6`: 3.5x
...
- `B_QUICK_BALL_MODIFIER` - Type: Generation Toggle
    - Quick Ball's catch multiplier is [More Info] 
        - \>= `GEN_5`: 5x
        - <= `GEN_4`: 4x
...
- `B_LURE_BALL_MODIFIER` - Type: Generation Toggle
    - Lure Ball's catch multiplier is [More Info] 
        - \>= `GEN_7`: 5x
        - <= `GEN_6`: 3x
...
- `B_HEAVY_BALL_MODIFIER` - Type: Generation Toggle
    - Heavy Ball's formula uses the formula from [More Info] 
        - \>= `GEN_7`: Generation 7
        - \>= `GEN_4`: Generation 4
        - = `GEN_3`: Generation 3
...
- `B_DREAM_BALL_MODIFIER` - Type: Generation Toggle
    - Dream Ball's catch multiplier when the target is asleep or has the ability Comatose is [More Info] 
        - \>= `GEN_8`: 4x
        - <= `GEN_7`: 1x
...
- `B_SPORT_BALL_MODIFIER` - Type: Generation Toggle
    - Sport Ball's catch multiplier is [More Info] 
        - \>= `GEN_8`: 1x
        - <= `GEN_7`: 1.5x
...
- `B_TERRAIN_BG_CHANGE` - Type: True / False
    - When `TRUE`, when terrain changes, the battle background changes to match until the change fades. [More Info] 
...
- `B_THUNDERSTORM_TERRAIN` - Type: True / False
    - When `TRUE` _and_ the overworld's weather is set to `WEATHER_RAIN_THUNDERSTORM`, Electric Terrain will be permanently set when a battle begins. [More Info] 
...
- `B_TERRAIN_TYPE_BOOST` - Type: Generation Toggle
    - Terrain boosts the damage of their corresponding moves by [More Info] 
        - \>= `GEN_8`: 30%
        - <= `GEN_7`: 50%
...
- `B_SECRET_POWER_EFFECT` - Type: Generation Toggle
    -  The effects for Secret Power uses the formula from: [More Info] 
        - \>= `GEN_6`: Generation 6
        - = `GEN_5`: Generation 5
        - = `GEN_4`: Generation 4
        - = `GEN_3`: Generation 3
...
- `B_SECRET_POWER_ANIMATION` - Type: Generation Toggle
    - The animation for Secret Power uses the formula from: [More Info] 
        - \>= `GEN_7`: Generation 7
        - = `GEN_6`: Generation 6
        - \>= `GEN_4`: Generation 4
        - = `GEN_3`: Generation 3
...
- `B_NATURE_POWER_MOVES` - Type: Generation Toggle
    - The move called by Nature Power uses the formula from [More Info] 
        - \>= `GEN_7`: Generation 7
        - = `GEN_6`: Generation 6
        - = `GEN_5`: Generation 5
        - = `GEN_4`: Generation 4
...
- `B_CAMOUFLAGE_TYPES` - Type: Generation Toggle
    - The type assigned by the move Camouflage uses the formula from [More Info] 
        - \>= `GEN_5`: Generation 5
        - = `GEN_4`: Generation 4
        - = `GEN_3`: Generation 3
...
- `B_ABILITY_POP_UP` - Type: True / False
    - Abilities are announced via [More Info] 
...
- `B_FAST_INTRO` - Type: True / False
    - Battle intro text prints [More Info] 
...
- `B_FAST_HP_DRAIN` - Type: True / False
    - The speed at which HP drains from the HP Bar is [More Info] 
...
- `B_SHOW_TARGETS` - Type: True / False
    - When `TRUE` _and_ a move has multiple targets, the targets will all be shown when the cursor is hovering over said move in the move menu. [More Info] 
...
- `B_SHOW_SPLIT_ICON` - Type: True / False
    - When `TRUE`, an icon on the Move's summary to indicate the move's classification is shown. [More Info] 
...
- `B_HIDE_HEALTHBOX_IN_ANIMS` - Type: True / False
    - When `TRUE`, health boxes are shown during move animations. [More Info] 
...
- `B_WAIT_TIME_MULTIPLIER` - Type: Set Value
    - This value is the base number of frames for pauses within battles - lower values will result in faster battles. Long, Medium, and Short pauses multiply this value by 4, 3 and 2 respectively. The default value in `pokeemerald` is 16. [More Info] 
...
- `B_SEMI_INVULNERABLE_CATCH` - Type: Generation Toggle
    - Pokemon that are in a semi-invulnerale state cannot be targeted for Pokeball capture. [More Info] 
        - \>= `GEN_4`: True
        - = `GEN_3`: False
...
- `B_CATCHING_CHARM_BOOST` - Type: Set Value
    - When the player has the Catching Charm, this value increses the odds of a Critical Capture by N%. [More Info] 
...
- `B_CRITICAL_CAPTURE` - Type: True / False
    - When `TRUE`, Critical Capture is enabled. [More Info] 
...
- `B_LAST_USED_BALL` - Type: True / False
    - When `TRUE`, "Last Used Ball" is enabled. [More Info] 
...
- `B_DOUBLE_WILD_CHANCE` - Type: Set Value
    - When a wild battle is initiated, this value is the N% chance that the battle will be a Double Battle. [More Info] 
...
- `B_MULTI_BATTLE_WHITEOUT` - Type: Generation Toggle
    - During a Multi Battle, when the player has no Pokemon available to fight, but their partner has Pokemon available to fight, the battle [More Info] 
...
- `B_EVOLUTION_AFTER_WHITEOUT` - Type: Generation Toggle
    - When a Pokemon has reached the criteria for evolution during battle, but then the player then loses the battle, the Pokemon will [More Info] 
...
- `B_WILD_NATURAL_ENEMIES` - Type: True / False
    - When `TRUE` _and_ wild Pokemon that are naturally antagonistic to each other (according to the Pokedex) are partnered in a Double Battle, they will attack each other. [More Info] 
...
- `B_AFFECTION_MECHANICS` - Type: True / False
    - When `TRUE`, in Battle Friendship Bonuses are active. [More Info] 
...
- `B_TRAINER_CLASS_POKE_BALLS` - Type: Generation Toggle
    - Each Trainer Class will send out their Pokemon [More Info] 
...
- `B_OBEDIENCE_MECHANICS` - Type: Generation Toggle
    - Obedience restrictions are placed on Pokemon whose OT belongs to the player, based on the level the Pokemon was met. [More Info] 
        - \>= `GEN_8`: True
        - <= `GEN_7`: False
